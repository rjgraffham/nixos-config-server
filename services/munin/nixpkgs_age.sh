...
case ${1:-} in
  config)
    ...
    echo 'nixpkgs_age.info Nixpkgs stable input age.'
    echo 'nixpkgs_unstable_age.info Nixpkgs unstable input age. (REMOVED)'
...
echo "scale=2; ($(date +%s) - $NIXPKGS_LAST_MODIFIED) / (60 * 60 * 24)" | bc
